...
- **"Nothing is signed" written in five place and three still say it.** macOS `.app` *is* self-signed — for TCC, not Gatekeeper. Corrected: `README.md`. Still wrong: `src-tauri/Cargo.toml:34` ("code signing, which this project does not have") and `RELEASING.md:175` ("the app is unsigned on macOS"). Two more read wrong but are right: `release.yml:158` describe **download page**, `RELEASING.md:98` describe **`.dmg`**, which genuine unsigned. **Fifth place sit in another repo** — site download page still say "Nothing here is code-signed" (`download.unsignedTitle` / `unsignedBody` in `spacetrace-website/src/i18n/ui/en.ts`, five locale). Change signing behaviour and move all five together, site included.
- **`RELEASING.md` still describe removed build triggers**: `desktop-continuous` tag on every push to `main`, and `paths-ignore` filter for `*.md`, `tasks/**` and `design-preview.html`. Neither exist in `release.yml` since 19 September 2026.
